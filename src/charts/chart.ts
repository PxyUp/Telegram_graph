...
export class PyxChart {
    ...
    drawCurrentSlice() {
        ...
        const getYCord = (value: number): number => {
            return (this.height - DEFAULT_SPACING) - value / (this.maxValue - realMinValue) * (this.height - 2 * DEFAULT_SPACING) 
        }
        ...
        for(let index = this.sliceStartIndex; index <= this.sliceEndIndex; index += sliceXSize) {
            ...
            const text = generateText({
                x: 2 * DEFAULT_SPACING + (this.width - 2 *DEFAULT_SPACING) / (DEFAULT_DAY_COUNT + 1) * currentIndex,
                y: (this.height),
            },
                label,
                [classNameAbsLine]
        ...
        Object.keys(this.columnsVisible).forEach((key) => {
            ...
            if (columnVisible) {
                ...
                const path = generatePath(this.columnDatasets[key].slice(this.sliceStartIndex, this.sliceEndIndex + 1).map((point, index) => {
                    return {
                        x: getXCord(index), 
                        y: getYCord(point)
                    }
                }).filter(item => item.x <= this.width - DEFAULT_SPACING), this.dataset.colors[key])
